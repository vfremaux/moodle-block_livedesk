...
// Form controller
  	...
  	if($data = $livedeskform->get_data()){
		...
		if($data->livedeskid == 0){
          	...
          	$data->livedeskid = insert_record('block_livedesk_instance', $data) ;
      	} else {
          	$livedesk = get_record('block_livedesk_instance', 'id', $data->livedeskid);
			$data->id = $data->livedeskid;
          	$data->servicestarttime = $data->servicestarttime_h * 3600 + $data->servicestarttime_m * 60;
          	$data->serviceendtime = $data->serviceendtime_h * 3600 + $data->serviceendtime_m * 60;
          	update_record('block_livedesk_instance', $data);          
      	}
      	
      	delete_records('block_livedesk_modules', 'livedeskid', $data->livedeskid);
	...
	if ($livedeskid > 0){               
  		...
  		$data = new StdClass;
	   	$data->livedeskid = 0;
	   	$data->bid = $bid;
	   	$data->course = $course->id; // maintain course context
