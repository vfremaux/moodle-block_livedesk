<?php // $Id: reply.php,v 1.2 2013-02-01 17:11:14 vf Exp $
 ...
 */
    ...
    if($queue_rec->locked == 1 && ($queue_rec->lockedby != $USER->id)){
        print(get_string('messagealreadylocked','block_livedesk'));
        exit;
    }
    ...
    if (!empty($forum)) {      // User is starting a new discussion in a forum
        ...
        if (!empty($confirm) && confirm_sesskey()) {    // User has confirmed the delete
            ...
            if ($post->totalscore) {
                ...
                if (! $post->parent) {  // post is a discussion topic as well, so delete discussion
                    ...
                    if ($forum->type == 'single') {
                        ...
                        $discussionurl = "reply.php?d=$post->discussion&replydone=1&reply={$reply}&cmid={$cmid}";
                    }
            ...
            if ($replycount) {
                ...
                print_header();
                notice_yesno(get_string('deletesureplural', 'forum', $replycount+1), "post.php?delete=$delete&amp;confirm=$delete&amp;sesskey=".sesskey(),
                ...
                print_header();
                notice_yesno(get_string("deletesure", "forum", $replycount), "post.php?delete=$delete&amp;confirm=$delete&amp;sesskey=".sesskey(),
                             $CFG->wwwroot.'/mod/forum/discuss.php?d='.$post->discussion.'#p'.$post->id);
                forum_print_post($post, $discussion, $forum, $cm, $course, false, false, false);
            }
        ...
        if (!empty($name) && confirm_sesskey()) {    // User has confirmed the prune
            ...
            redirect(forum_go_back_to("reply.php?d=$newid&reply={$reply}&cmid={$cmid}"));
    ...
    $mform_post = new mod_forum_reply_form('reply.php', array('course'=>$course, 'cm'=>$cm, 'coursecontext'=>$coursecontext, 'modcontext'=>$modcontext, 'forum'=>$forum, 'post'=>$post));
    ...
    if ($fromform = $mform_post->get_data()) {
        ...
        if ($fromform->edit) {           // Updating a post
            ...
            if ($forum->type == 'single') {
                ...
                $discussionurl = "reply.php?d=$discussion->id#p$fromform->id&replydone=1&reply={$reply}&cmid={$cmid}";
            }
            ...
            if ($fromform->id = forum_add_new_post($addpost, $message)) {
                ...
                delete_records('block_livedesk_queue','itemid', $fromform->id);
                
               	//update status of replied message 
                $replyed_post = get_record('block_livedesk_queue', 'itemid' , $reply);
                ...
                update_record('block_livedesk_queue', $replyed_post);
                ...
                if ($forum->type == 'single') {
                    ...
                    $discussionurl = "reply.php?d=$discussion->id&replydone=1&reply={$reply}&cmid={$cmid}";
                }
                add_to_log($course->id, 'forum', 'add post', "$discussionurl&amp;parent=$fromform->id", "$fromform->id", $cm->id);

                // print_continue(forum_go_back_to("$discussionurl#p$fromform->id"));
